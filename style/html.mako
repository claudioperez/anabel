<%
  ...
  from pdoc.html_helpers import extract_toc,\
   glimpse, to_html as _to_html, to_markdown as _to_markdown,\
   format_git_link
  ...
  def to_html(text):
    md_text = _to_markdown(text, docformat=docformat, module=module, link=link)
    ## print(md_text)
    return Pandoc(
      "--mathjax",
      "--citeproc",
      write="html",
      ## read="markdown+raw_html-markdown_in_html_blocks+raw_attribute",
      bibliography=["docs/elle.bib"],
      ## csl="docs/style.csl"
    ) << md_text
...
<%def name="show_module(module)">
  ...
  <%def name="show_func(f)">
    <dt id="${f.refname}"><code class="sourceCode hljs python name flex">
        <%
