...
class FEDEAS_Writer(ModelWriter):
    ...
    def dump_connectivity(self):
        ...
        for i, node in enumerate(Domain.nodes):
            script += "XYZ({},:) = [{}];\n".format(
                i + 1, " ".join(f"{x:8.8}" for x in node.coords)
            )
    ...
    def dump_constraints(self):
        ...
        for i, node in enumerate(Domain.nodes):
            if 1 in node.rxns:
                ...
                rx = " ".join(f"{i:4}" for i in node.rxns)
                script += "BOUN({:3},:) = [{}];\n".format(nd, rx)
    ...
    def dump_elements(self):
        ...
        for i, elem in enumerate(Domain.elems):
            script += "\n" + "ElemName{{{}}} = '{}';".format(
                i + 1, FEDEASmap[elem.type if hasattr(elem, "type") else type(elem)]
            )
        ...
        for i, elem in enumerate(Domain.elems):
            ...
            script += "ElemData{{{}}}.Np   = {};\n".format(i + 1, elem.Qpl[0, 0])

            script += "ElemData{{{}}}.Geom = 'GL';\n".format(i + 1, elem.Qpl[0, 0])

            if hasattr(elem, "Iz"):
                script += "\nElemData{{{}}}.I    = {};\n".format(i + 1, elem.Iz)

            if elem.nv > 1:
                script += "ElemData{{{}}}.Mp   = {};\n".format(i + 1, elem.Qpl[1, 0])
                rel = [1 if rel else 0 for rel in elem.rel.values()]
                script += "ElemData{{{}}}.Release = [{}; {}; {}];\n".format(
                    i + 1, rel[0], rel[1], rel[2]
                )
